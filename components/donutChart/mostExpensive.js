...
export default function MostExpensive() {
    ...
    useEffect(() => {
        fetch("https://customanalytic.leonardhors.site/api/subcategory/"+sessionStorage.getItem("ProductSubcategoryKey")+"/products/expensive/"+sessionStorage.getItem("ChartLimit_MostExpensive"))
        .then(res => res.json())
            .then(
            (result) => {
                //Default config
                if(sessionStorage.getItem("ChartLimit_MostExpensive") == null){
                    sessionStorage.setItem("ChartLimit_MostExpensive", "10");
                }
    ...
    function setLimit(limit){
        sessionStorage.setItem("ChartLimit_MostExpensive", limit);
        location.reload();
    }
                    ...
                    <ul className="dropdown-menu" aria-labelledby="dropdownMenuButton1">
                        <li>
                            <a className="dropdown-item">
                                ...
                                <input type="number" className='form-control' min="2" max="10" defaultValue={sessionStorage.getItem("ChartLimit_MostExpensive")} onBlur={(e)=> setLimit(e.target.value)}></input>
                            </a>
                        </li>
                    </ul>
                </div>
